let trades=JSON.parse(localStorage.getItem("sahilTrades")||"[]");let equityChart,strategyChart,instrumentChart;let calDate=new Date();
...
function renderAnalytics(){let s=groupPnl("strategy"),i=groupPnl("instrument");if(strategyChart)strategyChart.destroy();if(instrumentChart)instrumentChart.destroy();strategyChart=new Chart($("strategyChart"),{type:"bar",data:{labels:Object.keys(s),datasets:[{label:"P&L",data:Object.values(s)}]},options:{plugins:{legend:{display:false}}}});instrumentChart=new Chart($("instrumentChart"),{type:"bar",data:{labels:Object.keys(i),datasets:[{label:"P&L",data:Object.values(i)}]},options:{plugins:{legend:{display:false}}}});let ins=[];if(trades.length){let best=Object.entries(s).sort((a,b)=>b[1]-a[1])[0];let inst=Object.entries(i).sort((a,b)=>b[1]-a[1])[0];ins.push(`<div class="insight">🏆 Strongest strategy by P&L: <b>${best?.[0]}</b> (${money(best?.[1]||0)}).</div>`);ins.push(`<div class="insight">📊 Best instrument by P&L: <b>${inst?.[0]}</b> (${money(inst?.[1]||0)}).</div>`);let f=trades.filter(t=>t.emotion==="FOMO"||t.emotion==="Revenge");if(f.length)ins.push(`<div class="insight">🧠 ${f.length} trade(s) were marked FOMO/Revenge. Review the reason and lesson fields before repeating the setup.</div>`)}$("insights").innerHTML=ins.join("")||"Add some trades to generate insights."}
